...
const FROM_EMAIL = process.env.RESEND_FROM || 'Cliqo <onboarding@resend.dev>';
...
<body>
  <div class="wrapper">
    <div class="card">
      ...
      <div class="footer">
        <p>© 2025 Cliqo • This email was sent for account security. Do not share your code.</p>
      </div>
    </div>
  </div>
</body>
...
export const sendRegistrationOtp = async (toEmail, code) => {
  const html = baseTemplate(`
    ...
    <p>Hi there! Welcome to <strong>Cliqo</strong>. Please verify your email address to complete your registration.</p>
    <div class="otp-box">
      <div class="otp-code">${code}</div>
      <div class="otp-label">Your verification code</div>
    </div>
    <div class="expires">⏱ This code expires in <strong>10 minutes</strong>. Do not share it with anyone.</div>
    <p>If you didn't create a Cliqo account, you can safely ignore this email.</p>
  `);

  return sendEmail({ to: toEmail, subject: 'Verify your email — Cliqo', html });
};

export const sendWorkEmailOtp = async (toEmail, code) => {
  const html = baseTemplate(`
    <div class="badge">💼 Work Verification</div>
    <p>You requested to verify your <strong>work email</strong> on Cliqo. Use the code below to complete verification and unlock your work badge.</p>
    <div class="otp-box">
      <div class="otp-code">${code}</div>
      <div class="otp-label">Work email verification code</div>
    </div>
    <div class="expires">⏱ This code expires in <strong>10 minutes</strong>. Do not share it with anyone.</div>
    <p>Once verified, your profile will display a verified work badge, helping you stand out on Cliqo.</p>
  `);

  return sendEmail({ to: toEmail, subject: 'Verify your work email — Cliqo', html });
};

export const sendCollegeEmailOtp = async (toEmail, code) => {
  const html = baseTemplate(`
    <div class="badge">🎓 Student Verification</div>
    <p>You requested to verify your <strong>student / college email</strong> on Cliqo. Use the code below to get your student verified badge.</p>
    <div class="otp-box">
      <div class="otp-code">${code}</div>
  ...
  return sendEmail({ to: toEmail, subject: 'Verify your student email — Cliqo', html });
};
